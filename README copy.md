...
Deploy Bug Fix !!!
If your project works locally but does not work on Vercel, here is the solution. The current solution is to switch to MongoDB using an Atlas account (image uploads will still work since we use a different approach). If you don't have an Atlas account, please watch the following video:

Navigate to mongodb.com and sign up or log in if you already have an Atlas account.

Choose the free tier option.

Whitelist your IP address to allow connections from anywhere.

Go to your database, click 'Connect,' copy the database URL, and paste it into the DATABASE_URL field, replacing the placeholder password.

Remove the directUrl from schema.prisma and set the provider to MongoDB.

Update the id field to:
![alt text](image.png)

7. If you get a warning for any secondary ID (e.g., propertyId), make it an ObjectId like so:
